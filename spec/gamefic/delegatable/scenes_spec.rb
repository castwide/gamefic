...
describe Gamefic::Delegatable::Scenes do
  ...
  let(:object) do
    klass = Class.new do
      ...
      attr_accessor :scenebook
      define_method(:stage) { |*args, &block| block.call(*args) }
    end

    klass.new.tap do |obj|
      obj.scenebook = Gamefic::Scenebook.new(stage_func)
    end
  end
  ...
  describe '#on_conclude' do
    it 'adds an on_conclude block' do
      ...
      expect(object.scenebook.conclude_blocks).to be_one
    end
  end
  ...
  describe '#on_player_output' do
    it 'adds an on_player_output block' do
      ...
      expect(object.scenebook.player_output_blocks).to be_one
    end
  end
end
