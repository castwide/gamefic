module Gamefic
	...
	Action.new("go", Context::PROXIMATE.reduce(Portal)) { |actor, portal|
	...
	Action.new("exit") { |actor|
		...
		if portals.length > 1
			...
			actor.tell "\n#{actor.parent.longname}"
		else
			actor.tell "You don't see any exit."
		end
	}

	Parser.translate("go to [direction]", "go [direction]")
	Parser.translate("walk [direction]", "go [direction]")
	Parser.translate("move [direction]", "go [direction]")
